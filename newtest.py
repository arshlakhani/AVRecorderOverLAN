...
def record_audio():
    ...
    # List available devices and select the appropriate one
    devices = list_audio_devices()
    dev_index = None
    for index, name, max_channels in devices:
        print(f"Device {index}: {name} (Max Channels: {max_channels})")
        if max_channels >= chans:
            dev_index = index
            break
    ...
    print(f"Using Device {dev_index}: {devices[dev_index][1]}")
